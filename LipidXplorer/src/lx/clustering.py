    ...
    def setLinkageMethod(self, method):
      ...
      """
        if method == "single":
            ...
            raise ValueError(
                "distance method must be one of single, complete, average of uclus"
            )
    ...
    def cluster(self, matrix=None, level=None, sequence=None):
      ...
      """
        ...
        while len(matrix) > 2 or matrix == []:
            ...
            cluster = Cluster(
                level, self._data[smallestpair[0]], self._data[smallestpair[1]]
            )
            ...
            self._data.remove(
                self._data[max(smallestpair[0], smallestpair[1])]
            )  # remove item 1
            self._data.remove(
                self._data[min(smallestpair[0], smallestpair[1])]
            )  # remove item 2
   ...
   """
    ...
    def __init__(self, data, distance=None):
      ...
      """
        ...
        if len(data) > 1 and isinstance(data[0], TupleType):
            ...
            for item in data[1:]:
                if len(item) != control_length:
                    raise ValueError(
                        "Each item in the data list must have the same amount of dimensions. Item",
                        item,
                        "was out of line!",
                    )
        ...
        if (
            len(data) > 1
            and not isinstance(data[0], TupleType)
      ...
      """
        ...
        if n <= 1:
            raise ClusteringError(
                "When clustering, you need to ask for at least two clusters! You asked for %d"
                % n
            )
        ...
        if n > self.__initial_length:
            raise ClusteringError(
                """Unable to generate more clusters than items
...
def hclusterHeuristic(listSamples, dictSamples, resolution, distance=L1dist):
    ...
    for sample in listSamples:
        for peak in dictSamples[sample].listPrecurmass:
            clust.append(cluster_node(peak.precurmass, peak.intensity, sample))
            if peak.precurmass > greatestMass:
                greatestMass = peak.precurmass

    greatestDistance = greatestMass / resolution

    for index in range(len(clust)):
        clust[index].id = index

    closest = 0
    while closest < greatestDistance:
        lowestpair = (0, 1)
        closest = distance(clust[0].mass, clust[1].mass)
        ...
        for i in range(len(clust)):
            for j in range(i + 1, len(clust)):
                ...
                if (clust[i].id, clust[j].id) not in distances:
                    ...
                    # if dist < res * 2:
                    distances[(clust[i].id, clust[j].id)] = dist
        ...
        list = clust[lowestpair[1]].list + clust[lowestpair[0]].list
        # mergedmass = [sum(list) / len(list)]
        mergedmass = sum(list) / len(list)
...
def extract_clusters(clust, resolution):
    # extract list of sub-tree clusters from hcluster tree with distance<dist
    clusters = {}
    if clust.distance < (clust.mass / resolution):
        ...
        cr = []
        if clust.left != None:
            cl = extract_clusters(clust.left, resolution=resolution)
        if clust.right != None:
            cr = extract_clusters(clust.right, resolution=resolution)
        return cl + cr
...
def get_cluster_elements(clust):
    ...
    if clust.id > 0:
        ...
        return [[clust.mass, clust.intensity]]
    else:
        # check the right and left branches
        cl = []
        cr = []
