...
def lpdxImportCLI():
    ...
    optParser = OptionParser(
        usage="Usage: lxrun.py [options] masterscan.sc [output.csv]\n"
    )
    ...
    optParser.add_option(
        ...
        "--ini",
        dest="ini",
        help="path of the *.ini FILE if different from './lpdxImportSettings.ini'",
        metavar="FILE",
    )
    optParser.add_option(
        "-s",
    ...
    optParser.add_option(
        ...
        dest="dumpMasterScan",
        action="store_true",
        help="switch for dumping the content of the MasterScan",
        default=False,
    )
    ...
    optParser.add_option(
        "-c",
        "--complement",
        dest="complementMasterScan",
        action="store_true",
        help="generates the complement MasterScan and saves it in the origin directory",
        default=False,
    )
    optParser.add_option(
        "-q",
        ...
        type="string",
        metavar="ARG",
        help='the list of queries to run. Format is "query1.mfql, query2.mfql, ..."',
        default="",
    )
    optParser.add_option(
        "--tab",
    ...
    optParser.add_option(
        "--isocorrectMS",
        dest="isotopicCorrectionMS",
        action="store_true",
        help="switch isotopic correction on",
        default=False,
    )
    optParser.add_option(
        "--isocorrectMSMS",
        dest="isotopicCorrectionMSMS",
        action="store_true",
        help="switch isotopic correction on",
        default=False,
    )
    ...
    if cliOptions.masterScan is None:
        try:
            ...
            print("ERROR: A masterscan file was not specified. Use the option")
            print(" -m.")
    ...
    # overwrite the masterScan if given at the CLI
