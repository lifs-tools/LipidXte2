...
def t_UNDERSCORE(t):
    ...
    raise SyntaxErrorException(
        "In query %s, line %d: No underscore allowed."
        % (mfqlObj.filename, t.lexer.lineno),
        mfqlObj.filename,
        "",
        0,
    )
...
def p_tolerancetype(p):
    ...
    if float(p[1]) <= 0.0:
        raise LogicErrorException(
            "Tolerance value as given in the query '%s' can not be smaller"
            % mfqlObj.queryName
            + " or equal zero"
        )
...
def p_bterm(p):
    ...
    if mfqlObj.parsePart == "report":
            ...
            if se.listVariables != []:
                ...
                for (
                    ...
                    for key in list(vars.keys()):
                        if (
                            mfqlObj.queryName
                            == key.split(mfqlObj.namespaceConnector)[0]
                        ):
                            isIn = True
                            break
                    ...
                    if isIn:
                        ...
                        if False:
                            for empty_var in mfqlObj.dictEmptyVariables[
                                mfqlObj.queryName
                            ]:
                                list_empty_vars = mfqlObj.dictEmptyVariables[
                                    mfqlObj.queryName
                                ][empty_var]
        ...
        p[0] = report
        print(
            "%.2f sec. for %d comparisons"
            % (time.perf_counter() - start_time, count)
        )
...
def p_scan_object(p):
    ...
    try:
        ...
        raise SyntaxErrorException(
            'Variable "%s" in query "%s" does not exist.'
            % (p[1].variable, mfqlObj.queryName),
            mfqlObj.filename,
            0,
        )
    ...
    if isinstance(
        ...
        if scConst:
            if p[3] == "MS1+" or p[3] == "MS1-":
                ...
                p[3] == "MS2+"
                and mfqlObj.dictDefinitionTable[mfqlObj.queryName][
                    v
                ].elementSequence.polarity
                == 1
            ):
                massrange = scConst.getMassRange()
                mfqlObj.dictDefinitionTable[mfqlObj.queryName][v].options[
...
def startParsing(
    ...
    mfqlObjIn,
    ms,
    isotopicCorrectionMS,
    isotopicCorrectionMSMS,
    complementSC,
    parent,
    progressCount,
    generateStatistics,
