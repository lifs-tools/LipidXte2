           ...
           LineChart< Number, Number > extraChart, MasterXmlPane masterXmlPane, ObservableList< FAAnion > mFaAnionsList, QuantificationPane quantificationPane )
   {
      ...
      quantifyButton.setOnAction( new EventHandler< ActionEvent >()
      {
         @Override public void handle( ActionEvent event )
         {
            ...
            if ( removeReferenceCheckBox.isSelected() )
               additionalOptions.add( Quant.AdditionalOption.RemoveReference );
            if ( summarizeNCECheckBox.isSelected() )
               additionalOptions.add( Quant.AdditionalOption.SummarizeNCE );
            if ( noCorrectionCheckBox.isSelected() )
               additionalOptions.add( Quant.AdditionalOption.NoCorrection );
            if ( groupOnlyCheckBox.isSelected() )
               additionalOptions.add( Quant.AdditionalOption.GroupOnly );
            //				if(mergeUnspecifiedIsomerCheckBox.isSelected()) additionalOptions.add( Quant.AdditionalOption.MergeUnspecifiedIsomer );
            if ( intensityRatioCheckRemoveCheckBox.isSelected() )
               additionalOptions.add( Quant.AdditionalOption.IntensityCheckRemove );
            if ( mergeGlobalHomogeneousCheckBox.isSelected() )
               additionalOptions.add( Quant.AdditionalOption.MergeGlobalHomogeneous );
            //				if(applyTXCFinSummary.isSelected()) additionalOptions.add( Quant.AdditionalOption.ApplyTXCFinSummary );
            if ( removeIsomerInfoCheckBox.isSelected() )
               additionalOptions.add( Quant.AdditionalOption.RemoveIsomerInfo );
