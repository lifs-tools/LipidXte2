...
public class FragmentCollection
{
	...
	public void clear()
	{
		...
		fragments.values().forEach( c -> { c.values().forEach( i -> i.clear() ); c.values().clear(); } );
	...
	public void addCycles( int cycleCount, CycleCollection cycleCollection, int consecutiveNumber, int mmu )
	{
		...
		for( Double mz : highMzSet )
		{
			...
			removeList.stream().filter( a -> a >= min && a <= max ).forEach( item ->
							fragments.get( mz ).keySet().stream().filter( cycle ->
									fragments.get( item ).containsKey( cycle ) ).forEach( cycle ->
											merge( fragments.get( item ).get( cycle ), fragments.get( mz ).get( cycle ) )
							)
			);
		}
			...
			for ( Integer cycle : inputFragments.get( mz ).keySet() )
			{
				...
				for ( Float collisionEnergy : collisionEnergyList )
				{
					if ( inputFragments.get( mz ).containsKey( cycle ) )
						ticMap.get( cycle ).addIntensity( collisionEnergy,
							inputFragments.get( mz ).get( cycle ).get( collisionEnergy ) );
				}
			}
		}
...
//			System.out.println( "[" + from + ", " + to + "]" );
	...
	public TreeMap< Double, TreeMap<Integer, Fragment> > normalize( TreeMap< Double, TreeMap<Integer, Fragment> > data,
			int firstBasisPoints, int maximaThreshold )
	{
		...
		for ( Double mz : data.keySet() )
		{
			...
			for ( Integer cycle : data.get(mz).keySet() )
			{
				...
				for(int i = 1; i < ceList.length - 1; i++)
					fragment.put( ceList[i], 0.25f * data.get(mz).get(cycle).get( ceList[i-1] )
							+ 0.50f * data.get(mz).get(cycle).get( ceList[i] )
							+ 0.25f * data.get(mz).get(cycle).get( ceList[i+1] ) );
			}
		}
