...
def main(class_name, dirpath):
	...
	root = dom.getroot()
	scans = root.findall(".//{http://sashimi.sourceforge.net/schema_revision/mzXML_3.0}scan")
	...
	for scan in scans:
		...
		if m and n and rt < hcd: # there is an hcd and a retention time
	...
	templateFile = r'lipidXplorer/resources/template_project_' + class_name + '.lxp'
	...
	for hcd in hcdStarts:
		...
		print("*** are the mfql files where it says in the template project? ***")


if __name__ == '__main__':
	print('please provide class name and a folder with .mzXML files to generate lipidxplorere project files ')
