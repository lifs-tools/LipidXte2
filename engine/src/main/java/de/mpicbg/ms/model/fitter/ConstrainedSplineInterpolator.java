 ...
 * (ref: https://jetcracker.wordpress.com/2014/12/26/constrained-cubic-spline-java/)
 */
public class ConstrainedSplineInterpolator extends SplineInterpolator
{
   public PolynomialSplineFunction interpolate( double x[], double y[], double w[] ) throws DimensionMismatchException, NumberIsTooSmallException, NonMonotonicSequenceException
   {
      ...
      for ( int i = 1; i <= n; i++ )
      {
         ...
         d[ i ] = ( f2b - f2a ) / ( 6d * dx[ i - 1 ] ) * sigma[ i ];
         c[ i ] = ( x[ i ] * f2a - x[ i - 1 ] * f2b ) / ( 2d * dx[ i - 1 ] ) * sigma[ i ];
