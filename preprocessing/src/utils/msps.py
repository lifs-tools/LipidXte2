...
def main():
    ...
    if args.prefilter:
        preFiltered_filterlines = {
            ...
            for k, v in filterLines.iteritems()
        }
    ...
    if not args.prefilter and not args.preliminaryReductionFilter:
        preFiltered_filterlines = {
            k: preliminaryFilter(v) for k, v in filterLines.iteritems()
        }
    if args.generateCSV:
    ...
    if args.binsTheoResolution:
        filterlines_withBins = {
            ...
            for k, v in preFiltered_filterlines.iteritems()
        }
    if (
        ...
        filterlines_withBins = {
            k: generateBins_decimalPlaces(v)
            for k, v in filterLines.iteritems()
        }
    if args.generateCSV:
        filterlinePeaks_csv(
    ...
    if args.inWindowBin:
        filterlines_inBins = {
            ...
            for k, v in filterlines_withBins.iteritems()
        }
    if args.inFirstBin:
        filterlines_inBins = {
    ...
    filterlines_binData = {
        k: aggregateBinData(v) for k, v in filterlines_inBins.iteritems()
    }
    if args.generateCSV:
        filterlineBins_csv(filterlines_binData, "aggregateBinData.csv")

    filterlines_filtered = {
        k: filterBins(v) for k, v in filterlines_binData.iteritems()
    }
    if args.generateCSV:
        filterlineBins_csv(filterlines_filtered, "filterBins.csv")

    filtered_peaks = {
        k: formatPeaks(v) for k, v in filterlines_filtered.iteritems()
    }
