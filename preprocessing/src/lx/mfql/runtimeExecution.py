...
class TypeMFQL:
    ...
    def addEmptyVariable(self, name, variable):
        ...
        if variable.__class__.__name__ == "TypeList":
            raise LipidXException(
                "Using a list without 'IDENTIFY' is not" + " supported yet."
            )
    ...
    def genVariables_old(self, surveyEntry, plus_permutation=False):
        ...
        for se in loopResults:
            ...
            length_fragments = (
                len(list(se.dictMarks.values())) - 1 - len(noSFConstraint)
            )
            result = combinations_with_replacement(
                list(masses.keys()), length_fragments
            )
...
class TypeResult:
                ...
                if se.isInQueryMS(query.name, self.mfqlObj.namespaceConnector):
                    ...
                    for vars in se.listVariables:
                        ...
                        for definition in list(
                            self.mfqlObj.dictDefinitionTable[query.name].keys()
                        ):
                            if (
                                definition not in vars
                            ):  # and not self.mfqlObj.currVars[defintion] is None:
                                isIn = False
            ...
            if listSE[entry].isTakenBySuchthat:
                res = listSE[entry].precurmass / (
                    (
                        ...
                        * self.options["MSresolutionDelta"]
                    )
                    + self.options["MSresolution"].tolerance
                )
            ...
            if listSE[entry].listMark != []:
                ...
                for charge in list(chargePrecurmassSF.keys()):
                    ...
                    while delta <= 5 * cID:
                        ...
                        if 3 * cID <= delta + res and 3 * cID >= delta - res:
                            if nextKey.listMark != []:
                                M3found.append(nextKey)
                            else:
                                ## check if the intensity of the found peak is close to the expected
                        ...
                        if 1 * cID <= delta + res and 1 * cID >= delta - res:
                            if nextKey.listMark != []:
                                M1found.append(nextKey)
                            else:
                                ## check if the intensity of the found peak is close to the expected
                    ...
                    if (
                        ...
                        if Debug("isotopicCorrection"):
                            dbgstr = "\n- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -"
                            dbgstr += (
                                "\n MS sum compositions for %.4f:"
                                % actualKey.precurmass
                            )
                            str = "\n"
                            for scomp in chargePrecurmassSF[charge]:
                                str += " # %s" % repr(scomp)
            ...
            for msmsEntry in listSE[entry].listMSMS:
                if (
                    ...
                    if msmsEntry.dictBeforeIsocoIntensity == {}:
                        msmsEntry.dictBeforeIsocoIntensity = deepcopy(
                            msmsEntry.dictIntensity
                        )

                    for sample in list(listSE[entry].dictIntensity.keys()):
                        msmsEntry.dictIntensity[sample] = (
                            msmsEntry.dictIntensity[sample]
                            / listSE[entry].monoisotopicRatio
                        )
                        pass
            ...
            for key, value in [(k, dictPIS[pis][k]) for k in dictPIS[pis]]:
                ...
                while actualKey - precurKey - selectWin / 2 < 4.1:
                    ...
                    if (
                        ...
                        and 3.0099 >= actualKey - precurKey - selectWin / 2
                    ):
                        M3found.append(actualKey)

                    if (
                        2.0066 <= actualKey - precurKey + selectWin / 2
                        and 2.0066 >= actualKey - precurKey - selectWin / 2
                    ):
                        M2found.append(actualKey)

                    if (
                        1.0033 <= actualKey - precurKey + selectWin / 2
                        and 1.0033 >= actualKey - precurKey - selectWin / 2
                    ):
        ...
        for entry in range(len(listSE) - 1):
            if listSE[entry].listMark != []:
                ...
                for msmse in actualKey.listMSMS:
                    for mark in msmse.listMark:
                        if (
                            not mark.chemsc is None
                            and not mark.nlsc is None
                            and not mark.frsc is None
                        ):
                ...
                for pis in list(listM[0].keys()):
                    ...
                    if (
                        ...
                        if Debug("isotopicCorrection"):
                            ...
                            dbgstr += "\n"
                            dbgstr += "\nPrecur: %s %s" % (
                                listM[0][pis][0].se[0].precurmass,
                                listM[0][pis][0].se[0].listPrecurmassSF,
                            )

                            dbgstr += "\nPIS: %s" % (pis)
                            dbgout(dbgstr)

                        (Mtx, monoisotopic) = isotopicValuesInter(
                            elemC,
                            elemH,
                            elemO,
                            elemN,
                        ...
                        for i in varsShort:
                            for j in varsShort:
                                if i != j:
                                    if re.match(i, j):
                                        raise LipidXException(
                                            "vars: %s <-> %s. No variable name is allowed to be contained in another one"
                                            % (i, j)
                                        )
                        ...
                        if isinstance(report[2], type([])):
                            ...
                            for entry in report[2]:
                                ...
                                if isinstance(entry, TypeVariable):
                                    ...
                                    raise LipidXException(
                                        "Error with data type in REPORT"
                                    )

                                if tmpRpt is None:
                                    raise LipidXException(
                                        "Something wrong in query %s with "
                                        "variable %s."
                                        % (self.mfqlObj.queryName, entry.name)
                                    )
                            ...
                            try:
                                strCorr = noString.sub(
                                    r"['\g<1>']", strReportVars.strip('"')
                                )
                                try:
                                    str = eval(report[1] + " % " + strCorr)
                                except TypeError as detail:
                                    dbgout(
                                        "TypeError in report %s: %s"
                                        % (report[0], detail)
                                    )
                                    str = ""
                                ...
                                detail = "At variable %s in REPORT: %s" % (
                                    report[0],
                                    detail,
                                )
            ...
            if hasattr(query, "dataMatrix"):
                ...
                for smplIntensity in intensitySamples:
                    ...
                    for sample in sampleKeys:
                        ...
                        for x in data:
                            try:
                                dictRelativeAbundance[sample].append(
                                    x / dictTotalIonCount[sample]
                                )
            ...
            while (
                ...
                if (
                    ...
                    if (
                        ...
                        listSCMS[index + lookahead].isobaric.append(
                            listSCMS[index].name
                        )
                    # else:
    ...
    def removePermutations(self):
        ...
        for query in self.dictQuery:
            ...
            for idx, i in sorted(
                enumerated,
                key=lambda variable: sum_lv_err_and_intens(variable[1]),
                reverse=True,
            ):  # abs(list(variable[1].items())[0][1].errppm)):
            ...
            if len(listVar) > 0:
                ...
                for idx1, i in listVar:
                    ...
                    if not isIn:
                        listVar_noPermutations.append((idx1, i))

                self.dictQuery[query].listVariables = []
                for _, i in sorted(
                    listVar_noPermutations, key=lambda variable: variable[0]
                ):
...
class DataPoint:
    def __init__(self, indexKey, rowKey, data):
...
class TypeDataMatrix:
    ...
    def sumFattyAcids(self, faRow, indicatorRow):
        ...
        fa = self.getIntensity(
            faRow, list(self.dataL.keys())[0]
        )  # take the fatty acid entries of the first column
        for key in fa.rowKey:
            self.dataRKeys.append(
                "SUM_%s" % key
            )  # generate the keys for the sum of the fatty acids

        while index < len(list(self.dataL.values())):
            n = 1
            fa1 = self.getIntensity(
                faRow, indexDic[str(index)]
            )  # fa1 is from DataPoint type
            sum = fa1.data

            try:
                while (
                    list(self.dataL.values())[index + n][indicatorIndex]
                    == list(self.dataL.values())[index][indicatorIndex]
                ):
