    ...
    for polarity in listPolarity:
        ...
        for count in range(numLoops):
            ...
            while current < (len(listMSSpec[count]) - 1):
                ...
                lastEntry = None
                while (
                    listMSSpec[count][current].mass - lrsltMSMS[0].mass
                    < partialRes
                ):
                ...
                for i in lrsltMSMS:
                    for k in i.dictIntensity.keys():
                        ...
                        if not k in dictIntensity:
                            dictIntensity[k] = i.dictIntensity[k]
                        else:
                            dictIntensity[k] += i.dictIntensity[k]

                        if not k in dictScans:
                ...
                if count != numLoops - 1:
                    ...
                    for e in lrsltMSMS:
                        for p in e.peakList:
                            if len(p[1].keys()) < 2 and not listMSSpec[
                                count + 1
        ...
        if listClusters:
            ...
            if msmsLists != {}:
                ...
                if listSECharge != []:
                    ...
                    try:
                        currentAvg = iterListAvg.__next__()
                    except StopIteration:
                        print("No MS/MS spectra after the averaging!")
                        break

                    try:
                        nextAvg = iterListAvg.__next__()
                    except StopIteration:
                        onlyOneMSMS = True
...
def mkMSMSEntriesHeuristic_new(
    ...
    for polarity in listPolarity:
        ...
        if msmsThere:
            ...
            listClusters = heuristicAlignment(
                dictSpecEntry.keys(), dictSpecEntry, tolerance
            )  # ,
            # merge = mergeListMsms,
            # mergeTolerance = scan.options['MSMSresolution'],
            # mergeDeltaRes = scan.options['MSMSresolutionDelta'])
...
def linearAlignment(
    ...
    for entry in listResult[-1]:
        ...
        for i in entry[1]:  # entry[1] contains the merged specEntries
            ...
            if (
                not i.content["sample"] in cluster
...
def sparseHierarchicalAlignment(
    ...
    for index_sample in range(len(dictSamples.keys())):
        for index_peak in range(len(dictSamples.values()[index_sample])):
            spectrum.append(
                (
                    dictSamples.values()[index_sample][index_peak],
...
def alignPIS(
    ...
    for key in sc.listSamples:
        ...
        for msmsEntry in sc.dictSamples[key].listMsms:
            ...
            if not precurmass in dictScans:
        ...
        dictPISResult = {}
        for mass in sorted(
            dictScans.keys(), cmp=lambda x, y: cmp(float(x), float(y))
        ):
