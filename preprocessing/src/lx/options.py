...
class Options:
        ...
        if not self.isEmpty("optionalMStolerance"):
            ...
            if m is None:
                if (
                    ...
                    m = re.match(
                        r"(\d+|\d+.\d+)", self.options["optionalMStolerance"]
                    )
                    if not m is None:
        ...
        if not self.isEmpty("optionalMSMStolerance"):
            ...
            if m is None:
                if (
                    ...
                    m = re.match(
                        r"(\d+|\d+.\d+)", self.options["optionalMSMStolerance"]
                    )
                    if not m is None:
                        if float(m.group(1)) < 0.0:
                            raise LipidXException(
    ...
    def testOptions(self):
        ...
        if not self.isEmpty("MStolerance"):
            ...
            if m is None:
                if (
                    ...
                    m = re.match(
                        r"(\d+|\d+\.\d+)", self.options["MStolerance"]
                    )
                    if not m is None:
                        if float(m.group(1)) < 0.0:
                            raise LipidXException(
        ...
        if not self.isEmpty("MSMStolerance"):
            ...
            if m is None:
                if (
                    ...
                    m = re.match(
                        r"(\d+|\d+\.\d+)", self.options["MSMStolerance"]
                    )
                    if not m is None:
                        if float(m.group(1)) < 0.0:
                            raise LipidXException(
        ...
        if not self.isEmpty("alignmentMethodMS"):
            if not self.options["alignmentMethodMS"] in [
                "linear",
                "heuristic",
            ]:
                raise LipidXException(
    ...
    def formatOptions(self):
        ...
        if not self.isEmpty("MSresolution"):
            self.options_formatted["MSresolution"] = TypeTolerance(
                "res", float(o["MSresolution"])
            )
        if not self.isEmpty("MSMSresolution"):
