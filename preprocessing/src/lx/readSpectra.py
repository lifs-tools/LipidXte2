...
def add_Sample(sc=None, specFile=None, specDir=None, options={}, **kwargs):
    ...
    if len(ms1Scans) > 0:
        ...
        for ms1scan_entry in ms1Scans:
            ...
            listTotIonCurrent.append(
                float(ms1scan_entry["totIonCurrent"])
            )  # statistics
                ...
                if (
                    ...
                    if polarity != "":
                        if polarity != entry.get("polarity", None):
                            raise LipidXException(
                                "Having different polarities in one"
...
def add_DTASample(
    ...
    if sampleDir != "":
        if re.compile(r".*\%sneg_.*$" % (os.sep)).match(sampleDir):
            polarity = -1
        else:
            polarity = 1
    else:
