...
class mzFile(mzAPImzFile):
    ...
    _xp_ms2 = etree.XPath(
        './mz:cvParam[@name="ms level"][@value="2"]',
        namespaces=NSd,
        smart_strings=False,
    )
    _xp_pre = etree.XPath(
        (
            ".//mz:selectedIonList/mz:selectedIon/"
    ...
    _xp_frg_pis = etree.XPath(
        ('./mz:cvParam[@name="analyzer scan offset"]/' "attribute::value"),
        namespaces=NSd,
        smart_strings=False,
    )
    ...
    def scan_list(
        ...
        if self._info_file:
            return [
                (i["time"], i["mz"])
                for i in self._info_scans
                if (
                    start_time <= i["time"]
                    and ((not stop_time) or i["time"] <= stop_time)
                )
            ...
            if xt:
                ...
                if start_time <= time <= stop_time:
                    ...
                    if fstr:  # is there a filter line
                        ...
                        polarity = re.match(r".*([+-])\s.*", fstr[0]).group(1)
                        try:
                            # precursor ion scan
                            precursor = float(
                                re.match(
                                    r".*Full pr ([0-9]+\.[0-9]+)\s.*", fstr[0]
                                ).group(1)
                            )
                    ...
                    if self._xp_ms2(elem):
                        ...
                        if (
                            ...
                            if self._xp_iso(elem):
                                ...
                                print(
                                    "Skipping scan: '%s' ; Could not find a precursor or fragment!"
                                    % (elem.get("id"))
                                )
                        else:
                            p = precursor
    ...
    def scan_time_from_scan_name(self, scan_name):
        ...
        if self._info_file:
            return self._info_scans.filter(
                list_key="scan_name", value_list=[scan_name]
            )[0]["time"]
    ...
    def xic(self, start_time, stop_time, start_mz, stop_mz, filter=None):
        ...
        if self._info_file:
            items = [
                item
                for item in self._info_scans
                if start_time <= item["time"] <= stop_time
            ]
            if not items:
    ...
    def _build_info_scans(self):
        ...
        self._info_scans = mzInfoFile(
            dict(list(zip(keys, si + (offsets[i], sizes[i]))))
...
class mzFileInMemory:
        ...
        for s in self.fileobj.iterfind(
            ...
            if st is not None:
                ...
                if start_time <= time <= stop_time:
                    ...
                    if m is not None:
                        p = s.find(
                            './/%sselectedIonList/%sselectedIon/%scvParam[@name="selected ion m/z"]'
                            % (NS * 3)
                        )
                        if p is not None:
    ...
    def scan(self, time):
        ...
        min_cv = min(
            (cv for cv in self.fileobj.iterfind(search_term)),
    ...
    def xic(self, start_time, stop_time, start_mz, stop_mz, filter=None):
        ...
        search_term = (
            './/%srun/%sspectrumList/%sspectrum/%scvParam[@name="ms level"][@value="1"]'
            % (NS * 4)
        )
