...
def getInputFiles(importDir, options):
    ...
    for root, dirs, files in os.walk(importDir):
        ...
        if not (re.match(r"(^\.\w+).*|.*\.svn.*", root)) and options[
            "spectraFormat"
...
def doImport(
    ...
    if Debug("logMemory"):
        print(
            "ML> spectra loaded and averaged:", memory_logging.pythonMemory()
        )
...
def doImport_alt(
    ...
    listClusters = linearAlignment(
        dictSpecEntry.keys(), dictSpecEntry, scan.options["MSresolution"]
    )
